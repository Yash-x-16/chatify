...
const sizeStyle = {
    "sm": " rounded-xl active:translate-y-1  shadow-neutral-600 hover:bg-shadow-neutral-600   flex-item-center w-auto px-6 py-2  hover:scale-110  transition delay-50 duration-250 ease-in-out hover:translate-z-1 cursor-pointer h-auto shadow-md" , 
...
export  function Button(prop:buttonVariant){
    return <button onClick={prop.onClick} className={`${variantStyle[prop.variant]} ${sizeStyle[prop.size]} ${DefaulStyles}${prop.fullwidth?"w-max":null} `}>
       ...
       <span className={`m-auto  transition-colors text-white ${prop.size==="lg"?"font-bold":"font-medium "}`}>
        {prop.text}
       </span>
